---
title: 第 4 章 代码块、影子变量和控制结构
description: 理解 Go 的词法作用域与变量遮蔽，并正确选择 if、for、switch、标签和 goto。
---
...
本章围绕“一个名字在何处表示哪个对象”与“程序下一步执行哪段代码”展开。前半部分从代码块和作用域解释影子变量，说明短变量声明为什么既方便又容易隐藏外层变量；后半部分梳理 `if`、`for`、`switch`、标签和 `goto` 的控制规则。

读完后，应能沿着词法嵌套判断标识符的含义，识别意外遮蔽，按数据和退出条件选择循环形式，并准确分析 `break`、`continue` 和分支匹配的目标。文中还补充了当前 Go 对整数、迭代器函数及逐次迭代变量的规则。

> 核对日期：2026 年 9 月 13 日；实验环境为 Go 1.26.5、Windows/AMD64。
>
> 整数 `range` 和逐次迭代变量语义需要 Go 1.22 或更新的语言版本；标准库 `math/rand/v2` 需要 Go 1.22 或更新的工具链；对迭代器函数使用 `range` 需要 Go 1.23 或更新的语言版本。
...
内层声明与外层声明同名时，内层标识符会暂时遮蔽外层标识符。外层变量没有被删除或修改，只是在内层声明的作用域中无法通过这个名字访问。
...
短变量声明 `:=` 是否复用变量，只看同一个块，而不是所有外层块。当前块中可以重新声明已有变量，但左侧必须至少有一个新的非空白变量，而且被重新声明的变量类型不能改变：
...
if x > 0 {
	x, z := 40, 50 // 在 if 的隐式块中，x 和 z 都是新变量
	fmt.Println(x, z) // 40 50
}
fmt.Println(x) // 20
```

混合处理返回值时尤其容易意外遮蔽。例如，外层已有 `err`，但内层块中的 `value, err := read()` 会声明一个新的 `err`。如果原意是更新外层变量，可以先声明真正的新变量，再使用普通赋值：
...
类似地，把局部变量命名为 `len`、`true` 或 `error` 可能合法，却会使相应的预声明标识符暂时不可用或改变表达式含义。应优先选择表达业务含义的名字；只有确实想让内层状态独立时，才有意使用遮蔽。编译器会报告类型错误和未使用变量，但不会把每一次合法遮蔽都视为错误，`go vet` 也不提供通用的影子变量检查。

## 4.3 `if` 语句

Go 的 `if` 条件必须产生 `bool`，条件外不写括号，执行体必须使用花括号。分支按顺序判断，只执行第一个条件为真的分支：
...
排列条件时要先处理更具体的情况。以整除判断为例，“同时被 2 和 3 整除”必须放在两个单独条件之前，否则前面的宽泛条件会先匹配，后面的分支永远没有机会执行。
...
`range` 按数据源产生零个、一个或两个迭代值。Go 1.26 支持以下数据源：
...
迭代器函数的签名必须是 `func(yield func() bool)`、`func(yield func(V) bool)` 或 `func(yield func(K, V) bool)`。循环每获得一次值，运行时就调用迭代器提供的 `yield`；循环提前结束后，`yield` 返回 `false`，迭代器也必须停止。标准库 `iter` 包定义了常用的 `Seq` 和 `Seq2` 类型，本书会在介绍函数和泛型之后再展开这种写法。
...
映射的迭代顺序未指定，同一个程序的不同轮遍历也可能不同，业务逻辑不能依赖观察到的顺序。遍历期间删除尚未到达的键，该键不会再产生；新加入的键可能产生，也可能被跳过。如果修改会影响控制逻辑，通常先收集键并按需要排序，再进行处理。
...
这项规则由包含该包的模块在 `go.mod` 中声明的 Go 语言版本控制，不只是由当前安装的工具链版本决定。声明 Go 1.21 或更早语言版本的包仍使用旧的复用语义；迁移时应先升级 `go` 指令并运行测试。没有所属模块时，编译器采用由构建环境确定的语言版本。
...
“每次迭代有新变量”也不改变 4.4.4 节的复制规则。新的 `word` 仍是当前元素的副本，修改它不会修改切片元素。
...
当 `switch` 位于循环内时，无标签 `break` 只结束 `switch`，不会结束外层循环：
...
表达式 `switch` 先计算可选的初始化语句，再计算标签表达式，然后按源码顺序寻找第一个匹配的 `case`。标签省略时默认为 `true`，形成无标签 `switch`。`default` 最多出现一次，可以放在任意位置，但只有没有其他分支匹配时才执行。
...
同一个 `case` 可以用逗号列出多个匹配表达式。标签表达式与各 `case` 表达式必须能够进行相等比较；切片、映射和函数不能直接用作普通 `switch` 的比较值。初始化语句声明的名字在所有分支中可见，而每个 `case` 和 `default` 又各自形成隐式块，因此不同分支可以声明同名的局部变量。

Go 默认只执行匹配分支，不需要在每个分支末尾写 `break`。空 `case` 什么也不做，也不会自动执行下一个分支。确实需要无条件进入下一分支时，可以把 `fallthrough` 写成当前分支最后一条非空语句；它不检查下一分支的条件，而且不能出现在最后一个分支中，通常重组共同逻辑会更清楚。

### 4.5.1 无标签 `switch`

无标签 `switch` 等价于以 `true` 为标签，每个 `case` 可以写独立的布尔表达式：
...
分支仍然只执行第一个匹配项，所以这里必须把能被 6 整除的情况放在能被 2 或 3 整除之前。无标签 `switch` 适合表达一组互相关联、互斥且优先级明确的条件。

### 4.5.2 在 `if` 和 `switch` 之间选择

只有一两个判断、各分支处理流程差异很大，或后续判断依赖前一分支的计算时，`if` 往往更直接。同一个值存在多个离散情况，或多个相关条件构成一组分类规则时，`switch` 通常更清楚。

不要为了减少行数把无关条件塞进同一个无标签 `switch`。选择标准不是语法是否可行，而是读者能否把这些分支理解成同一个决策。类型 `switch` 用于判断接口值的动态类型，将在接口章节讨论。
...
初始化语句中新 `x` 的作用域从短变量声明结束后开始，因此右侧 `x+1` 读取外层值 `10`。条件和 `if` 主体先使用隐式块中的 `x == 11`、`y == 20`。主体里的 `x := 30` 又在更内层的显式块声明一个 `x`，只遮蔽到右花括号。整个 `if` 结束后，两层内侧声明都已超出作用域，最后读取外层 `x == 10`。
...
遍历一个整数切片，对每个值应用以下规则：能同时被 2 和 3 整除时输出 `Six!`；否则，能被 2 整除时输出 `Two!`，能被 3 整除时输出 `Three!`，其余输出 `Never mind`。使用无标签 `switch` 编写循环，并解释分支顺序是否可以交换。
...
如果模块仍声明 Go 1.21 或更早语言版本，旧语义会复用循环变量，保存其地址可能得到不同结果。迁移时应升级语言版本并用测试检查依赖旧行为的代码。
